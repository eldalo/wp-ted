		...
		<div>
			<?php if ( have_posts() ): ?>
				...
				<article <?php post_class( 'cf' ) ?> id="post-<?php the_ID() ?>">
					...
					<div><?php wp_link_pages( array('before' => '<div class="page-links"><span class="page-links-title">' . __( 'Pages:', 'ted' ) . '</span>', 'after' => '</div>', 'link_before' => '<span>', 'link_after' => '</span>' ) ) ?></div>
					<div class="tags"><?php echo get_the_term_list( get_the_ID(), 'custom_tag', '<span class="tags-title">' . __( 'Custom Tags:', 'ted' ) . '</span> ', ', ' ) ?></div>
